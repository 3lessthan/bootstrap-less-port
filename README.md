...
The code is currently aligned with [Bootstrap v4.3.1](https://github.com/twbs/bootstrap/tree/v4.3.1). It requires Less v3.8.1 or above to compile.
...
3. **Loops** Where possible, Sass `@each` loops have been replaced with the Less [`each()`](http://lesscss.org/functions/#list-functions-each) function. Sass `@for` directives have been replaced by `each()` together with the [`range()`](http://lesscss.org/functions/#list-functions-range) function.
...
4. **Code style** While the code style is mostly identical to that used by Bootstrap, a few changes have been made:
